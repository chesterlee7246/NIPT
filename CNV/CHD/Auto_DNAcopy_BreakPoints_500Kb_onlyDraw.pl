...
# system("perl $p2/Each_Bin_CNV_info.pl $to $name");
 ...
 if(-e $t4) {system("perl $p3/Auto_CNV_Annotation_500Kb.pl $t4 $p3");}
 if(-e $t5) {system("perl $p3/Auto_CNV_Annotation_500Kb.pl $t5 $p3");}
 ...
 system("perl $p2/CytoBand_CNV_TO_SVG_500Kb.pl $p2/cytoBand.txt $YF $PID $info5_mosaic $id2{$index} $gender");
 }
 else{
 system("perl $p2/plot_chromosome_CNValue.pl $CNValue $PID $p2");
 system("perl $p2/plot_chromosome.pl $info4 $PID $p2 $label $id2{$index} $gender");
