...
while(<FH>)
{
 ...
 my @tmp=split /\t/,$_;
 $labelXY+=1;
 if($tmp[0] eq "chrY") {$Label+=1;}
 if($tmp[0] ne "chrX" && $tmp[0] ne "chrY") {$label+=1;}
 $Cnums{$tmp[0]}+=1;
 $info{$tmp[0]}{$tmp[1]}=$_."\t".$labelXY;
}
close(FH);
...
my $inputXY="";
my $outXY=$ARGV[0]; $outXY=~s/.txt$/_XY.png/;
for(my $i=0;$i<$gender;$i++)
{
 if($inputXY eq "") {$inputXY=$file{$chr[$i]};}
 else{
 $inputXY.=" ".$file{$chr[$i]};
 }
}
my $numsXY=0;
if($Label==0) {$numsXY=23;}
else {$numsXY=24;}
system("Rscript $path/plot_all_chromosome_CNValue.r $id $labelXY $numsXY $outXY $inputXY >/dev/null");

my $input="";
my $out=$ARGV[0]; $out=~s/.txt$/_Autosomal.png/;
for(my $i=0;$i<22;$i++)
{
...
my $nums=22;
system("Rscript $path/plot_all_chromosome_CNValue.r $id $label $nums $out $input >/dev/null");
