...
while(<F1>)
{
 ...
 my @tmp=split /\t/,$_;
 my $RR=2*(2**($tmp[5]));
 if($RR>=2.2 && $RR<2.75 && ($tmp[8]>=5.00 || $tmp[9]>=5.00) && $tmp[6]<=1e-5) {$record+=1;}
 elsif($RR<=1.8 && $RR>1.25 && ($tmp[8]>=5.00 || $tmp[9]>=5.00) && $tmp[6]<=1e-5) {$record+=1;}
...
while(<F1>)
{
 ...
 my $ebin=$tmp[11];
 my $rlen=$tmp[13];
 if($chr ne "chr19" && (($RR>=2.2 && $RR<2.75) | ($RR<=1.8 && $RR>1.25)) && ($pmad>=5.00 || $psd>=5.00) && $pvalue<=1e-5){
		my @region=();
		...
		print TO "$id	$chr	$start	$end	$nums	$logRR	$rbin	$ebin	$rlen	$info\n";
 }elsif($chr eq "chr19" && (($RR>=2.2 && $RR<2.75) | ($RR<=1.8 && $RR>1.25)) && ($pmad>=5.00 && $psd>=5.00) && $pvalue<=1e-5){
		my @region=();
		foreach my $pos1 (sort {$a<=>$b} keys %{$cytoband{$chr}}){
			foreach my $pos2 (sort {$a<=>$b} keys %{$cytoband{$chr}{$pos1}}){
				if(($start>$pos2) | ($end<$pos1)) {next;}
