...
chr20,63025520
chr21,48129895
chr22,51304566", col.names=c("chr", 'max'), header=FALSE)
	chroms$chr <- factor(chroms$chr, levels=paste("chr", c(1:22), sep=""))
...
plot_data <- function(df, segments, chroms, title_name){
...
df$chr <- factor(df$chr, levels=paste("chr", c(1:22), sep=""))
df <- df[order(df$chr, df$start), ]
segments$chrom <- factor(segments$chrom, levels=paste("chr", c(1:22), sep=""))
...
plot_data(newdf, segments, chroms, args[3])
