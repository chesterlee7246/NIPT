...
plot_data <- function(df, segments, chroms, title_name, gender){
        ...
        scale_y_continuous(limits=c(0, 4), breaks=seq(0, 4, 1), labels=c(" 0 ", ' 1 ', ' 2 ', ' 3 ',' 4 '), expand=c(0,0)) +
	...
	p <- p + theme(text=element_text(size=14,family="ArialNarrow"))
		...
		geom_hline(yintercept = seq(1,4,1),color='grey',linetype="dotted",size=0.2)+
#               geom_hline(yintercept = seq(1,7,1),color='grey',linetype="dotted",size=0.2)
                geom_hline(aes(yintercept=2), colour="#808080", linetype="dotdash",size=0.2)+
                geom_hline(aes(yintercept=2.8), colour="#606060", linetype="dotdash",size=0.2)+
                geom_hline(aes(yintercept=1.2), colour="#606060", linetype="dotdash",size=0.2)+
                geom_hline(aes(yintercept=2.1), colour="#606060", linetype="dotdash",size=0.2)+
                geom_hline(aes(yintercept=1.9), colour="#606060", linetype="dotdash",size=0.2)+
                geom_hline(aes(yintercept=0.2), colour="grey", linetype="dotted",size=0.2)+
                geom_hline(aes(yintercept=0.9), colour="grey", linetype="dotted",size=0.2)+
                geom_hline(aes(yintercept=1.1), colour="grey", linetype="dotted",size=0.2)
        png(sprintf("%s.png", title_name), width=2400, height=1000, res=180)
#        png(sprintf("%s.png", title_name), width=2400, height=1200, res=180)
