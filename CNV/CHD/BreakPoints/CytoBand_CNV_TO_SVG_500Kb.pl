...
while(<RS>)
{
 ...
 my @tmp=split /\t/,$_;
 if($tmp[8]<(500*1000) && ($_=~/Decipher_Syndrome/i || ($_=~/DECIPHER/i && $_=~/pathogenic/i) || ($_=~/ISCA/i && $_=~/pathogenic/i))) {
 my $cnvcolor="";
 my $RR=2*(2**($tmp[5]));
