...
my %png1=();
my %png2=();
my %cytoband_file=();          ###### add cytoband 020312              --tao
...
{	
	if($file=~/^IonXpress_0*(\d+)_/){
		...
		elsif($file=~/100Kb_Cytoband.txt$/){  $cytoband_file{$index}=$file; }    ###### add cytoband 020312   --tao
	}
}
closedir(DIR);
...
foreach my$index(sort {$a<=>$b} keys %png1)
{
	...
	###### add $appendix 020312   --tao
	my $cb_file=$result_dir."/".$cytoband_file{$index};    ###### add cytoband 020312   --tao
	open(FL,$cb_file);
	my $tmp_i=0;
	my $appendix='';
#	$appendix.="{";
...
#	    $tmp_i++;
		...
		$_=~s/[\r\n]//g;
		my @arr=split/\t/,$_;
		my ($len,$cn_info,$chrom,$cy_site,$rl_site,$var);
		if ($arr[8]< 1000000){
			$tmp_i++;
			my $len=($arr[8]/1000)."Kb";
			my $cn_info=$arr[13];
			if($cn_info=~/[dup|del]\((.*)\)\((.*)\)\.seq\[GRCh37\/hg19\]\((\d+\-\d+)\)X([1|3])/){
				$chrom=$1;
				$cy_site=$2;
				$rl_site=$3;
				$var='';
				...
				$appendix.="\'seq[hg19] $chrom$cy_site \($rl_site\)×$4\;$chrom$cy_site\,chr$chrom\:$rl_site\,$len\,$var\,临床意义未明\|\'";
#				$appendix.="\'seq[hg19] $chrom$cy_site \($rl_site\)×$4\;$chrom$cy_site\,$rl_site\,$len\,$var\,临床意义未明\|\'";        ###  tao 0327 遗传部要求去掉基因组位置中的染色体号
}
}
if($tmp_i==0 ){$appendix='-';}
print $appendix;
...
#				my $final_size = sprintf("%.2f",($size/1000000))."Mb";
	...
	if(not system("grep '$idx' $result_dir\/index2id_YCM.txt")){
		...
		print "$site $run_name $reads $Gb $len $isp $enrich $clonal $Final $Empty $no $Polyclonal $Low $instrument $idx $index $cnv $cnvsize $result_reading $Scatterplot $Cytoband $rawReads $uniqReads $select_y $Y_type $Y_size $Y_region $Y_pic $appendix\n";
       system("python $bin_dir/uploadCNVnewXiangYa.py $idx $reads $Gb $len $isp $enrich $clonal $Final $Empty $no $Polyclonal $Low $instrument $run_name $index $rawReads $uniqReads $cov $cnv $cnvsize $result_reading $Scatterplot $Cytoband $select_y $appendix '$Y_type' '$Y_size' '$Y_region' '$Y_pic' ");
...
#		system("python $p1/chd_rpc_SiteCommon.py $site $run_name $reads $Gb $len $isp $enrich $clonal $Final $Empty $no $Polyclonal $Low $instrument $idx $index $cnv $cnvsize $result_reading $Scatterplot $Cytoband $rawReads $uniqReads $select_y '$Y_type' '$Y_size' '$Y_region' '$Y_pic' ");
		...
		print "$site $run_name $reads $Gb $len $isp $enrich $clonal $Final $Empty $no $Polyclonal $Low $instrument $idx $index $cnv $cnvsize $result_reading $Scatterplot $Cytoband $rawReads $uniqReads $select_y $appendix\n";
	   system("python $bin_dir/uploadCNVnewXiangYa.py $idx $reads $Gb $len $isp $enrich $clonal $Final $Empty $no $Polyclonal $Low $instrument $run_name $index $rawReads $uniqReads $cov $cnv $cnvsize $result_reading $Scatterplot $Cytoband $select_y $appendix");
