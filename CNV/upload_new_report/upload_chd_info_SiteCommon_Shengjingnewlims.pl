...
#			push @arr_size, sprintf("%.2f",$CNV{$key}[0])."Mb";
	...
	if(not system("grep '$idx' $result_dir\/index2id_YCM.txt")){
		...
		print "$run_name $reads $Gb $len $isp $enrich $clonal $Final $Empty $no $Polyclonal $Low $instrument $idx $index $cnv $cnvsize $result_reading $Scatterplot $Cytoband $rawReads $uniqReads $select_y $Y_type $Y_size $Y_region $Y_pic \n";
		...
		print "$run_name $reads $Gb $len $isp $enrich $clonal $Final $Empty $no $Polyclonal $Low $instrument $idx $index $cnv $cnvsize $result_reading $Scatterplot $Cytoband $rawReads $uniqReads $select_y\n";
