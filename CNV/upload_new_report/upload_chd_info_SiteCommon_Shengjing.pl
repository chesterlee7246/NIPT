...
foreach my$index(sort {$a<=>$b} keys %png1)
{
	...
	my (%CNV,%efc_CNV, @aneuploid, @microCNV); #add %efc_CNV 20/2/26 ; change effctive cnv to efc_cnv --tao
	if(exists $report_file{$index}){
		...
		while(<CF>){
			...
			my $effectivesize = $tmp[8];
		 	my $size=$tmp[3]-$tmp[2]+1;

			my ($chr, $s) = @tmp[1,2];
			$chr =~ s/chr//;
			my $chr_chinese = $chr=~/[XY]/ ? $chr : $chr."号";
##########判断$effectivesize是否大于1M  200525
			if($effectivesize>=(1000*1000) || ($effectivesize<(1000*1000) && ($_=~/Decipher_Syndrome/i ||($_=~/Decipher/i && $_=~/pathogenic/i)||($_=~/ISCA/i && $_=~/pathogenic/i))) ){
...
#               $abs_size = $abs_size."Mb";
					...
					$efc_CNV{$cc}[0] +=$effectivesize/(1000*1000);  ## 20/2/26 --tao
...
#				my $final_size = sprintf("%.2f",($size/1000000))."Mb";
	...
	if(scalar(@arr_CNV)==0){
		...
		foreach my$key(@arr_CNV){
			push @arr_cnv, $key;
#	        my @temp=split/-/,$CNV{$key}[0];
#		    print $temp[0];
#			print $temp[1];
            print $CNV{$key}[0];print $efc_CNV{$key}[0];
			push @arr_size, sprintf("%.2f",$CNV{$key}[0])."Mb(".sprintf("%.2f",$efc_CNV{$key}[0])."Mb)";
...
#			push @arr_size, sprintf("%.2f",$CNV{$key}[0])."Mb";
	...
	if(not system("grep '$idx' $result_dir\/index2id_YCM.txt")){
        ...
        system("python $bin_dir/push_cnv_data.py $idx $reads $Gb $len $isp $enrich $clonal $Final $Empty $no $Polyclonal $Low $instrument $run_name $index $rawReads $uniqReads $cov $cnv $cnvsize $result_reading $Scatterplot $Cytoband $select_y '$Y_type' '$Y_size' '$Y_region' '$Y_pic' ");
...
#		system("python $p1/chd_rpc_SiteCommon.py $site $run_name $reads $Gb $len $isp $enrich $clonal $Final $Empty $no $Polyclonal $Low $instrument $idx $index $cnv $cnvsize $result_reading $Scatterplot $Cytoband $rawReads $uniqReads $select_y '$Y_type' '$Y_size' '$Y_region' '$Y_pic' ");
	   ...
	   system("python $bin_dir/push_cnv_data.py $idx $reads $Gb $len $isp $enrich $clonal $Final $Empty $no $Polyclonal $Low $instrument $run_name $index $rawReads $uniqReads $cov $cnv $cnvsize $result_reading $Scatterplot $Cytoband $select_y");
