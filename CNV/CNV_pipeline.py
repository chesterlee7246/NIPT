...
def lift_over(inbed,outbed,out_unmapbed):
    os.system("/home/bioinfo/local/bin/liftOver "+inbed+" /home/bioinfo/database/hg19/hg19_ucsc/hg19ToHg38.over.chain.gz "+outbed+" "+out_unmapbed)
    chmod_for_curator(inbed,outbed,out_unmapbed)

def file_add_hg38(infile):
    in_bed_file=os.path.dirname(infile)+"/"+os.path.basename(infile)[0:13]+"_temp_in_bed.txt"
    outbed=os.path.dirname(infile)+"/"+os.path.basename(infile)[0:13]+"_temp_out_bed.txt"
    out_unmapbed=os.path.dirname(infile)+"/"+os.path.basename(infile)[0:13]+"_temp_out_unmap_bed.txt"
    inf=open(infile,'r')
#    outf=open(in_bed_file,'w')
    dict_site=dict()
    for line in inf:
        line=line.strip()
        ...
        arr=line.split("\t")
        outf=open(in_bed_file,'w')
        outf.write(arr[1]+"\t"+arr[2]+"\t"+arr[3]+"\n")
        outf.close()
        lift_over(in_bed_file,outbed,out_unmapbed)
        inf1=open(outbed,'r')
        for line1 in inf1:
            line1=line1.strip()
            dict_site[arr[1]+"_"+arr[2]+"_"+arr[3]]=line1.split("\t")[1::]
        inf1.close()
    inf.close()
    os.system("rm "+outbed+" "+out_unmapbed)
    inf2=open(infile,'r')
    outf1=open(infile.replace(".txt","_hg38.txt"),'w')
    for each in inf2:
        each=each.strip()
        arr1=each.split("\t")
        if each.startswith("ID"):
            outf1.write("\t".join(arr1[0:2])+"\thg19_start\thg19_end\thg38_start\thg38_end\t"+"\t".join(arr1[4::])+"\n")
        else:
            outf1.write("\t".join(arr1[0:4])+"\t"+"\t".join(dict_site[arr1[1]+"_"+arr1[2]+"_"+arr1[3]])+"\t"+"\t".join(arr1[4::])+"\n")
    inf2.close()
    outf1.close()
    os.system("rm "+in_bed_file)
    chmod_for_curator(infile)
...
class Pipeline():
    ...
    def upload(self):
        normal = '/upload_new_report/upload_chd_info_SiteCommonnewlims.pl'
        upload_bin_dir =  self.pip_dir + "/upload_new_report"
        upload_bat = {
                'XiangYa':'/upload_new_report/upload_chd_info_SiteCommon_XiangYanewlims.pl',
                'ShengJing':'/upload_new_report/upload_chd_info_SiteCommon_Shengjingnewlims.pl',
                'SYTH':'/upload_new_report/upload_chd_info_SiteCommon_SYTHnewlims.pl'
                }
    ...
    def make_qc_report(self):
        ...
        with open(index2id, 'rt') as fh:
            for line in fh:
                ...
                mapd=''
                sd=''
                if sample_info['mapd'] >= 0.3 or sample_info['sd'] >=0.3:
                    noise_count += 1
...
"""
    ...
    def archive(self):
        ...
        for ff in file_names:
            if ff.endswith("Cytoband.txt") or ff.endswith("Report.txt"):
                try:
                    file_add_hg38(ff)
                    shutil.copy(ff.replace(".txt","_hg38.txt"), zip_dir)
