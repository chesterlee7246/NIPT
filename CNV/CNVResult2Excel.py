...
AllData = []
AllData.append(['RunNum','indexid','samplename','raw_reads','uniq_reads','uniq_rate','dup_reads','dup_frac','map_rate','coverage','sd','mapd','CNVs','sex','png'])
for RunNum in RunList:
    ...
    for idname in indexname:
        ...
        image_file=run_dir + "/" + indexid + "_rawlib_rmdup_MAPQ10_Fbin_GC_All_Merge_Normalized_LogRR_Extract_CNV_ConvertValue_XY.png"
        img_file=run_dir + "/" + indexid + "_rawlib_rmdup_MAPQ10_Fbin_GC_All_Merge_Normalized_LogRR_Extract_CNV_ConvertValue_XY.bmp"
        AllData.append([RunNum,indexid,samplename,str(raw_reads),str(uniq_reads),str(uniq_rate),str(dup_reads),str(dup_frac),str(map_rate),str(coverage),str(sd),str(mapd),CNVs,sex,img_file])
        if os.path.isfile(image_file):
            imagef=Image.open(image_file)
            imagef.save(img_file)
...
for i in range(0,len(AllData)):
    ...
    if i == 0:
        worksheet.write(i,14, AllData[i][14], style)
    else:
        worksheet.insert_bitmap(AllData[i][14],i,14,scale_x=0.1, scale_y=0.1)
